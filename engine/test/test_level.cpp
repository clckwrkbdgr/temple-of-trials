...
TEST(containers_should_be_impassable)
{
	Level level(2, 2);
	level.map.celltypes[0].passable = true;
	level.containers.push_back(Container::Builder().pos(Point(1, 1)));
	ASSERT(!level.is_passable(1, 1));
}
...
TEST(fountains_should_be_impassable)
{
	Level level(2, 2);
	level.map.celltypes[0].passable = true;
	level.fountains.push_back(Fountain::Builder().pos(Point(1, 1)));
	ASSERT(!level.is_passable(1, 1));
}
...
TEST(stairs_should_be_passable)
{
	Level level(2, 2);
	level.map.celltypes[0].passable = true;
	level.stairs.push_back(Stairs::Builder().pos(Point(1, 1)));
	ASSERT(level.is_passable(1, 1));
}
